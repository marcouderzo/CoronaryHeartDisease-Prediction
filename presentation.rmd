...
The p value is a proportion: if the p value is 0.05, that means that 5% of the time one would see a test statistic at least as extreme as the one they found if the null hypothesis was true.
...
Lasso regression is a regularization technique. It is used over regression methods for a more accurate prediction. This model uses shrinkage, that is where data values are shrunk towards a central point as the mean. The lasso procedure encourages simple, sparse models (i.e. models with fewer parameters).
...
```
There are some differences between what Lasso decided to remove and what we removed before through Variable Selection. Lasso removed RestingECG and RestingBP, while keeping Age and MaxHR.
...
Overall, we'll see through the Analysis of Deviance thatthe removal of such predictors from the model does not yield significantly worse performance.
...
Let's now analyze the findings of Ridge Regression:
...
In this case, the high p-value of 0.2043 tells us that the reduced model is not significantly different from the full model.
...
Naive Bayes seems to outperform all the above models in all metrics, with each metric being >= 90%. Therefore, for the problem of predicting Coronary Heart Disease using Statistical Learning methods, Naive Bayes is the best model to use.
