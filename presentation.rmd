...
#naivebayes.roc <- naivebayes.roc[[1]]
...
roc <- c(lda.roc, qda.roc, glm.model.4.roc, lasso.roc, ridge.roc, 0) # naivebayes.roc to be fixed.


model_performance <- matrix(c(acc, prec, rec, f1.score, roc), nrow = 6, ncol = 5, byrow = TRUE)
...
colnames(performance_summary) <- c("Model", "Accuracy", "Precision", "Recall", "F1 Score", "ROC")
...
Naive Bayes seems to outperform all the above models in all metrics, with each metric being >= 90%.
